...
from gesture_utils import gesture_feature
from expr_utils import expr_feature
...
class UpdateFeatures:
    def __init__(self, config, *args):
        ...
        self.expr_feature = expr_feature()
        self.gesture_feature = gesture_feature()
    ...
    def remove_files_to_be_reprocessed(self, dest_dict):
...
def main(args):
    ...
    update_features.remove_files_to_be_reprocessed(dest_dict)
    ...
    while not success:
        ...
        try:
            ...
            dest_clients = update_features.get_dest_dict()
            for client in dest_clients.keys():
                current_client = client
                if config["client"] == "all" or config["client"] == client:
                    logging.info(f"Postprocessing recordings for: , {client}")

                    # Check if face, skeleton and sound directories exist, create if not
                    update_features.create_feature_directories(config, client)
