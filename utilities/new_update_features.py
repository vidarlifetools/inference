...
class UpdateFeatures:
    ...
    def process_skeleton(self, client, file_id, dest_dict):
        logging.info(f"Processing skeleton for {client}, file id {file_id}")
        ...
        if not file_id in dest_dict[client]["skeleton"]:
            ...
            logging.info(f"Creating: , {skeleton_dir}")
    ...
    def process_sound(self, client, file_id, dest_dict, sound_annotations):
        logging.info(f"Processing sound for {client}, file id {file_id}")
...
def main(args):
    ...
    update_features = UpdateFeatures(config)
    ...
    dest_dict = update_features.get_dest_dict()
    for client in dest_dict.keys():
        if config["client"] == "all" or config["client"] == client:
            logging.info(f"Postprocessing recordings for: , {client}")
            update_features.remove_files_to_be_reprocessed(client)
            # Check if face, skeleton and sound directories exist, create if not
            update_features.create_feature_directories(client)

            # Loop through raw files
            if config["rec_id"] == "all":
                raw_files = [f for f in dest_dict[client]["raw"] if "_post.wav" in f]
            else:
                raw_files = [f for f in dest_dict[client]["raw"] if "_post.wav" in f and config["rec_id"] in f]

            for raw_file in raw_files:
                file_id = raw_file.split("_")[0]
                if "face" in config["postprocess"]:
                    update_features.process_face(client, file_id, dest_dict)
                if "skeleton" in config["postprocess"]:
                    update_features.process_skeleton(client, file_id, dest_dict)
                sound_annotations = update_features.get_sound_annotations()
                if "sound" in config["postprocess"]:
                    update_features.process_sound(client, file_id, dest_dict, sound_annotations)
        time.sleep(5)
