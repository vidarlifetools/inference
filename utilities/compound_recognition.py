...
from streams import get_class_stream, send_compound_stream
from utils import write_feature_vector


# TODO: Add transition sequences between annotated and not annotated sequences  - decide whether one or more transition
#  classes - other changes in definition of transition classes?
...
def compound_recognition_thread(sub_face_socket, sub_gesture_socket, sub_sound_socket, pub_socket, person,
                                base_for_person, frame_rate, logger):
    '''
    This function produces a continuous stream of compound predictions using the information related to the face
    expression, gesture and sound recognition trhreads.
    At the moment the predictions are output every CHUNK_PREDICTIONth frame. The results are saved as log messages in a
    logger file.

    :param sub_face_socket: socket for input from the face expression recognition thread
    :param sub_gesture_socket: socket for input from the gesture recognition thread
    :param sub_sound_socket: socket for input from the sound recognition thread
    :param pub_socket: socket for output
    :param person: name of catalogue for person
    :param base_for_person: full path to the catalogue with <person>
