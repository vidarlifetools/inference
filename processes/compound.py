...
class Compound(DataModule):
    ...
    def process_data_msg(self, msg):
        ...
        if type(msg) == FaceclassMessage:
            #self.logger.info(
            #    f"Face ({msg.timestamp}): processing time = {time.time() - msg.timestamp:.3f}, time since last = {time.time() - self.last_face_time:.3f}")
            self.last_face_time = time.time()
            self.face_class[self.e_in] = msg
        ...
        if self.g_in != self.g_out and self.e_in != self.e_out:
            # There are face and gesture classes present
            if abs(self.face_class[self.e_out].timestamp - self.gesture_class[self.g_out].timestamp) < 0.01:
                # TODO Call self.compound_predict(self.gesture_class[], self.face_class[], self.sound_class)
                ...
                self.logger.error(f"Timestamps does not match, {self.face_class}, {self.gesture_class}")
